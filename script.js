...
/* ==========================================================
   GA4 EVENT TRACKING
   Added for Stanlake Park Experience Hub.
   This block only listens for interactions and sends GA4 events.
   It does not change layout, image paths or page content.
   ========================================================== */
(function () {
  ...
  window.trackHubEvent = trackEvent;
  ...
  function getSectionName(el) {
    ...
    return section.id || section.className || "unnamed_section";
  }
  ...
  if ("IntersectionObserver" in window) {
    const sectionObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        ...
        const sectionName = section.id || section.className || "unnamed_section";
  ...
  /* Wine Matcher reveal */
  const matchButtonForTracking = document.getElementById("match-button");

  if (matchButtonForTracking) {
    matchButtonForTracking.addEventListener("click", () => {
      const selected = Array.from(
        document.querySelectorAll("#preference-options input:checked")
      ).map(input => input.value);

      trackEvent("wine_matcher_reveal", {
        selected_count: selected.length,
        selected_preferences: selected.join(", ")
      });
    });
  }

  /* Wine Matcher clear */
  const resetButtonForTracking = document.getElementById("reset-button");

  if (resetButtonForTracking) {
    resetButtonForTracking.addEventListener("click", () => {
      trackEvent("wine_matcher_clear");
    });
  }

  /* Experience Matcher completion */
  const experienceMatchButtonForTracking = document.getElementById("experienceMatchButton");

  if (experienceMatchButtonForTracking) {
    experienceMatchButtonForTracking.addEventListener("click", () => {
      const selected = Array.from(
        document.querySelectorAll(".experience-options button.active")
      ).map(button => button.dataset.value || getText(button));

      const recommendation =
        document.querySelector(".experience-result-content > h3") ||
        document.querySelector(".experience-result-card h3");

      trackEvent("experience_matcher_completed", {
        selected_count: selected.length,
      ...
      if (recommendation && selected.length) {
        trackEvent("experience_recommended", {
          experience: getText(recommendation),
          selected_options: selected.join(", ")
        });
      }
    });
  ...
  const clearExperienceButtonForTracking = document.getElementById("clearExperienceButton");

  if (clearExperienceButtonForTracking) {
    clearExperienceButtonForTracking.addEventListener("click", () => {
      trackEvent("experience_matcher_clear");
    });
  }

  /* Berkshire Guide category filters */
  document.querySelectorAll(".explorer-tab[data-category]").forEach(tab => {
    tab.addEventListener("click", () => {
  ...
  const clearExplorerFiltersForTracking = document.getElementById("clearExplorerFilters");

  if (clearExplorerFiltersForTracking) {
    clearExplorerFiltersForTracking.addEventListener("click", () => {
      trackEvent("guide_filter_clear");
    });
  }

  const explorerSearchForTracking = document.getElementById("explorerSearch");
  let explorerSearchTimer;

  if (explorerSearchForTracking) {
    explorerSearchForTracking.addEventListener("input", () => {
      window.clearTimeout(explorerSearchTimer);

      explorerSearchTimer = window.setTimeout(() => {
        const term = explorerSearchForTracking.value.trim();
  ...
  /* Stay finder */
  document.querySelectorAll("#party-size button, #stay-location button").forEach(button => {
    button.addEventListener("click", () => {
      trackEvent("stay_finder_option", {
        option: button.dataset.size || button.dataset.location || getText(button),
        group: button.closest("#party-size") ? "party_size" : "location"
      });
    });
  });

  /* Delegated click tracking for dynamic cards and links */
  document.addEventListener("click", event => {
    ...
    if (link) {
      ...
      if (link.classList.contains("wine-link")) {
        const card = link.closest(".result-card");
        const wineName = card ? getText(card.querySelector("h3")) : label.replace(/^View\s+/i, "").replace(/\s+→$/i, "");

        trackEvent("wine_selected", {
          wine_name: wineName,
          section: "wine_matcher"
        });
      }

      if (link.closest(".explorer-card")) {
        const card = link.closest(".explorer-card");

        trackEvent("guide_location_clicked", {
          location: getText(card.querySelector("h3")),
          category: getText(card.querySelector(".explorer-category"))
        });
      }

      if (link.closest(".stay-card") || link.classList.contains("property-button")) {
        const card = link.closest(".stay-card");

        trackEvent("accommodation_clicked", {
          property: card ? getText(card.querySelector("h3")) : label,
          location: section
        });
      }

      if (
        href.includes("stanlakepark.com/calendar") ||
        label.toLowerCase().includes("book") ||
        label.toLowerCase().includes("tour dates") ||
        link.classList.contains("tour-cta-button") ||
      ...
      if (href.includes("stanlakepark.com/weddings")) {
        trackEvent("wedding_interest", {
          label: label,
          section: section,
          destination: href
        });
    ...
    if (button && button.closest(".experience-options")) {
      trackEvent("experience_option_clicked", {
        option: button.dataset.value || getText(button)
      });
    }
  });
})();
