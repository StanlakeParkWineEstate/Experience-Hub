...
/* =========================================================
   GA4 event tracking for Stanlake Park Experience Hub
   Measurement ID is installed in index.html: G-SEP3K9GVB2
   This block is intentionally append-only.
   It does not change layout, image paths, classes or content.
   ========================================================= */
(function () {
  ...
  function getSectionName(section) {
    ...
    return cleanText(heading ? heading.textContent : "unnamed_section").toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "");
  }
  ...
  function checkScrollDepth() {
    ...
    const scrollPercent = ((window.scrollY + window.innerHeight) / pageHeight) * 100;
  ...
  // Meaningful engagement marker
  window.setTimeout(function () {
    trackEvent("engaged_120_seconds");
  }, 120000);

  // Click tracking, using event delegation so generated cards are included too.
  document.addEventListener("click", function (event) {
    const clicked = event.target.closest("a, button, label");
    if (!clicked) return;

    const text = cleanText(clicked.textContent);
    const href = clicked.getAttribute("href") || "";

    if (clicked.matches(".nav-book-button, .tour-cta-button") || href.includes("stanlakepark.com/calendar")) {
      trackEvent("book_click", {
        item: text || "Book Tour",
        location: getSectionName(clicked.closest("section")),
        destination: href || "internal"
      });
    }

    if (clicked.matches(".property-button") || href.includes("/booking/")) {
      const card = clicked.closest(".stay-card");
      const propertyName = cleanText(card ? card.querySelector("h3")?.textContent : text);
    ...
    if (clicked.matches(".wine-link")) {
      const card = clicked.closest(".result-card");
      const wineName = cleanText(card ? card.querySelector("h3")?.textContent : text.replace(/^View\s+/i, "").replace(/\s+→$/, ""));
    ...
    if (clicked.matches(".explorer-tab[data-category]")) {
      trackEvent("guide_filter", {
        category: clicked.dataset.category || text
      });
    }

    if (clicked.closest(".explorer-card") && clicked.matches("a")) {
      const card = clicked.closest(".explorer-card");
      trackEvent("guide_location_clicked", {
        location: cleanText(card.querySelector("h3")?.textContent),
        category: cleanText(card.querySelector(".explorer-category")?.textContent),
        destination: href
      });
    }
    ...
    if (clicked.matches("#match-button")) {
      const selected = Array.from(document.querySelectorAll("#preference-options input:checked")).map(function (input) {
        return input.value;
      });
      trackEvent("wine_matcher_used", {
        selected_count: selected.length,
        selected_preferences: selected.join(", ")
      });
    }

    if (clicked.matches("#experienceMatchButton")) {
      const selected = Array.from(document.querySelectorAll(".experience-options button.active")).map(function (button) {
        return button.dataset.value || cleanText(button.textContent);
      });
      ...
      window.setTimeout(function () {
        ...
        const resultTitle = result ? cleanText(result.querySelector("h3")?.textContent) : "";
        if (resultTitle) {
    ...
    if (clicked.closest("#party-size") || clicked.closest("#stay-location")) {
      trackEvent("stay_preference_selected", {
        choice: clicked.dataset.size || clicked.dataset.location || text
      });
    }
  ...
  // Search tracking for Berkshire Guide, debounced.
  ...
  if (explorerSearch) {
    explorerSearch.addEventListener("input", function () {
      ...
      searchTimer = window.setTimeout(function () {
        const term = cleanText(explorerSearch.value);
        if (term.length >= 2) {
          trackEvent("guide_search", { term: term });
        }
      }, 700);
    });
  }
