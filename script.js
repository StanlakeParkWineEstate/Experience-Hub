...
/* =========================
   GA4 EVENT TRACKING
   Added safely: no image paths, markup or styles are changed.
   ========================= */
(function () {
  ...
  function getSectionName(section) {
    ...
    return section.id || cleanText(section.querySelector("h1, h2, h3, .eyebrow")?.textContent) || "unnamed_section";
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (matchButton) {
      matchButton.addEventListener("click", function () {
        const selected = Array.from(document.querySelectorAll("#preference-options input:checked"))
    ...
    const experienceMatchButton = document.getElementById("experienceMatchButton");
    if (experienceMatchButton) {
      experienceMatchButton.addEventListener("click", function () {
        const selected = Array.from(document.querySelectorAll(".experience-options button.active"))
          .map(function (button) { return button.dataset.value || cleanText(button.textContent); });
        ...
        window.setTimeout(function () {
          ...
          const recommendation = cleanText(result?.querySelector("h3")?.textContent);
    ...
    const clearExperienceButton = document.getElementById("clearExperienceButton");
    if (clearExperienceButton) {
      clearExperienceButton.addEventListener("click", function () {
        trackEvent("experience_matcher_cleared");
      });
    }

    /* Stay finder */
    document.querySelectorAll("#party-size button, #stay-location button").forEach(function (button) {
      button.addEventListener("click", function () {
        trackEvent("stay_preference_selected", {
          type: button.dataset.size ? "party_size" : "location",
          value: button.dataset.size || button.dataset.location || cleanText(button.textContent)
        });
      });
    });

    /* Berkshire Guide categories */
    document.querySelectorAll(".explorer-tab[data-category]").forEach(function (tab) {
    ...
    /* Delegated click tracking for static and dynamic links */
    document.addEventListener("click", function (event) {
      const link = event.target.closest("a");
      if (!link) return;

      const text = cleanText(link.textContent);
      const section = getSectionName(link.closest("section"));
      const href = link.href || "";

      if (link.classList.contains("wine-link")) {
        const card = link.closest(".result-card");
        const wineName = cleanText(card?.querySelector("h3")?.textContent) || text.replace(/^View /, "").replace(/ →$/, "");
      ...
      if (link.classList.contains("property-button")) {
        const propertyCard = link.closest(".stay-card");
        trackEvent("accommodation_book_click", {
          property: cleanText(propertyCard?.querySelector("h3")?.textContent) || text,
          section: section
        });
        return;
      }
      ...
      if (
        ...
        /book|tour dates|find dates|calendar/i.test(text)
      ) {
      ...
      if (/wedding|show round/i.test(text) || /\/weddings\//i.test(href)) {
